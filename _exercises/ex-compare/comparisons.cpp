...
struct Gadget
{
    ...
    {
        return name == other.name && (std::strong_order(price, other.price) == 0);
    }
};
...
std::strong_ordering operator<=>(const RatingStar& ratingStar, const RatingValue& ratingValue)
{
    return ratingStar <=> RatingStar{ratingValue};
}
